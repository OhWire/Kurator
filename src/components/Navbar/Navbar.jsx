...
const Navbar = () => {
  ...
  return (
    <div className={`flex flex-col items-center w-full h-full bg-custom-gray border border-black border-opacity-15 bg-opacity-60 shadow-2xl rounded-2xl relative overflow-visible md:w-auto md:h-auto md:rounded-none md:bg-opacity-100 md:shadow-none ${isMenuOpen ? 'fixed inset-0 z-50 bg-white' : ''}`}> {/* Adjusted class for menu open */}
      <div className="flex  justify-between items-center w-full p-4 md:justify-center md:items-start md:pt-6 md:h-32"> {/* Adjusted flex for small screen */}
        <Link to="/">
          <button onClick={() => setIsMenuOpen(false)}> {/* Close menu on logo click */}
      ...
      <div className={`flex flex-col space-y-4 justify-center items-center w-full h-full ${isMenuOpen ? 'h-screen' : ''} md:pt-4 md:flex-row md:space-y-0 md:space-x-6`}> {/* Adjusted flex for menu open */}
        {menuItems.map(item => (
          <Link to={item.to} key={item.name} onClick={() => { setSelected(item.to); setIsMenuOpen(false); }}> {/* Close menu on item click */}
